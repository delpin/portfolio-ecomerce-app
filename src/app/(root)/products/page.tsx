  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-heading-3 text-dark-900">Products {totalCount ? `(${totalCount})` : ""}</h1>
        <Sort />
      </div>

      {hasFilters ? (
        <div className="mb-4 flex flex-wrap gap-2">
          {Object.entries(filters as Record<string, unknown>).flatMap(([k, v]) => {
            const arr = Array.isArray(v) ? v : v != null ? [v] as unknown[] : [];
